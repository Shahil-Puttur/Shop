...
async function initializeDatabase() {
    const client = await pool.connect(); try { await client.query('CREATE TABLE IF NOT EXISTS game_state (id INT PRIMARY KEY, play_count INT NOT NULL);'); await client.query(`CREATE TABLE IF NOT EXISTS recent_plays (id SERIAL PRIMARY KEY, fingerprint VARCHAR(255) NOT NULL, timestamp TIMESTAMPTZ NOT NULL);`); const res = await client.query('SELECT * FROM game_state WHERE id = 1'); if (res.rows.length === 0) { await client.query('INSERT INTO game_state (id, play_count) VALUES (1, 0)'); } console.log('Database initialized successfully.'); } catch (err) { console.error('DATABASE INITIALIZATION FAILED:', err.stack); } finally { client.release(); }
}
...
app.get('/', (req, res) => {
    // This message will appear in your Render logs every 5 minutes.
    console.log(`Ping received at ${new Date().toISOString()}. Server is awake. ✅`);
    res.send('<h1>Cafe Rite Backend is live! V-LEGENDARY</h1>');
});

app.post('/check', async (req, res) => { /* ... unchanged ... */ });
...
app.post('/check', async (req, res) => { const { fingerprint } = req.body; if (!fingerprint) return res.status(400).json({ error: 'Fingerprint required.' }); const client = await pool.connect(); try { await client.query("DELETE FROM recent_plays WHERE timestamp < NOW() - INTERVAL '24 hours'"); const result = await client.query('SELECT * FROM recent_plays WHERE fingerprint = $1', [fingerprint]); const lastPlayed = result.rows.length > 0 ? result.rows[0].timestamp : null; if(lastPlayed) { const cooldownEnd = new Date(lastPlayed).getTime() + (24 * 60 * 60 * 1000); return res.json({ canPlay: false, cooldownEnd: cooldownEnd }); } return res.json({ canPlay: true }); } catch(err) { console.error("Check error:", err); res.status(500).json({ error: 'Server error during check.' }); } finally { client.release(); } });
app.post('/play', async (req, res) => { const { boxIndex, fingerprint } = req.body; if (boxIndex === undefined || !fingerprint) { return res.status(400).json({ error: 'Box index and fingerprint are required.' }); } const client = await pool.connect(); try { const checkResult = await client.query('SELECT * FROM recent_plays WHERE fingerprint = $1', [fingerprint]); if (checkResult.rows.length > 0) { return res.status(403).json({ error: 'This device has already played today.' }); } await client.query('INSERT INTO recent_plays (fingerprint, timestamp) VALUES ($1, NOW())', [fingerprint]); const result = await client.query('UPDATE game_state SET play_count = play_count + 1 WHERE id = 1 RETURNING play_count'); const currentUserNumber = result.rows[0].play_count; const isWinner = winnerMilestones.includes(currentUserNumber); const balls = ['🏀', '⚾', '⚽', '🥎', '🏉', '🏈', '🏐', '🧶']; const shuffledBalls = balls.sort(() => Math.random() - 0.5); let finalItems = new Array(9).fill(null); if (isWinner) { finalItems[boxIndex] = '🍔'; let ballIndex = 0; for (let i = 0; i < 9; i++) { if (finalItems[i] === null) { finalItems[i] = shuffledBalls[ballIndex++]; } } } else { finalItems[boxIndex] = shuffledBalls.pop(); let remainingSpots = []; for (let i = 0; i < 9; i++) { if (i !== boxIndex) remainingSpots.push(i); } const burgerSpot = remainingSpots[Math.floor(Math.random() * remainingSpots.length)]; finalItems[burgerSpot] = '🍔'; for (let i = 0; i < 9; i++) { if (finalItems[i] === null) { finalItems[i] = shuffledBalls.pop(); } } } let responsePayload = { win: isWinner, items: finalItems }; if (isWinner) { const part1 = Math.floor(Math.random() * 90) + 10; const part2 = Math.floor(Math.random() * 9000) + 1000; responsePayload.winnerCode = `${part1}5964${part2}`; } res.json(responsePayload); } catch (err) { console.error('GAME LOGIC ERROR:', err.stack); res.status(500).json({ error: 'Server could not process game request.' }); } finally { client.release(); } });
