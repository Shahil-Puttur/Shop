...
// This is a professional-grade CORS setup that is more reliable.
const whitelist = ['https://cafe-rite.netlify.app', 'http://localhost:5500']; // Add your Netlify URL here
const corsOptions = {
...
async function initializeDatabase() { /* ... same as before ... */ }
app.get('/', (req, res) => res.send('<h1>Cafe Rite Backend is live and healthy! V2</h1>'));
app.get('/status', async (req, res) => { /* ... same as before ... */ });
app.post('/play', async (req, res) => { /* ... same as before ... */ });
app.get('/reset-for-my-bro', async (req, res) => { /* ... same as before ... */ });
...
app.get('/status', async (req, res) => { const client = await pool.connect(); try { const result = await client.query('SELECT game_version FROM game_state WHERE id = 1'); res.json({ version: result.rows[0].game_version || 1 }); } catch (err) { res.status(500).json({ error: 'Could not get game status.' }); } finally { client.release(); } });
const items = ['🍔', '🏀', '⚾', '⚽', '🥎', '🏉', '🏈', '🏐', '🧶'];
const winnerMilestones = [3, 23, 73, 123, 173, 223];
app.post('/play', async (req, res) => { const client = await pool.connect(); try { const result = await client.query('UPDATE game_state SET play_count = play_count + 1 WHERE id = 1 RETURNING play_count'); const currentUserNumber = result.rows[0].play_count; let isWinner = winnerMilestones.includes(currentUserNumber); const shuffledItems = [...items].sort(() => Math.random() - 0.5); const burgerIndex = shuffledItems.indexOf('🍔'); const randomSpot = Math.floor(Math.random() * 9); if (isWinner) { [shuffledItems[randomSpot], shuffledItems[burgerIndex]] = [shuffledItems[burgerIndex], shuffledItems[randomSpot]]; } let responsePayload = { win: isWinner, items: shuffledItems }; if (isWinner) { const part1 = Math.floor(Math.random() * 90) + 10; const part2 = Math.floor(Math.random() * 9000) + 1000; responsePayload.winnerCode = `${part1}5964${part2}`; } res.json(responsePayload); } catch (err) { console.error('GAME LOGIC ERROR:', err.stack); res.status(500).json({ error: 'Server could not process game request.' }); } finally { client.release(); } });
